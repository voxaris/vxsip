...
#define	SIP_REQUEST	"%s %s%s"

/* list of sent-by values registered by the UA */
sent_by_list_t *sip_sent_by = NULL;
int sip_sent_by_count = 0;

static int sip_add_name_aspec (_sip_msg_t *, char *, char *, char *, boolean_t, char *, char *);
...
int sip_copy_values (char *ptr, _sip_header_t * header)
{
   ...
   while (value != NULL)
   {
      if (value->value_state != SIP_VALUE_DELETED)
      {
         ...
         else
         {
            ...
            while (*e != SIP_COMMA)
               e--;
            len += value->value_start - e;
            (void) strncpy (p, e, len);
         }
         tlen += len;
         p += len;
      }
...
/* Returns number of bytes in the given int */
static int sip_num_of_bytes (int num)
{
   int num_of_bytes = 0;
 ...
 */
int sip_adjust_msgbuf (_sip_msg_t * msg)
{
   ...
   if (header != NULL)
   {
      ...
      header->sip_hdr_sipmsg->sip_msg_len -= header->sip_hdr_end - header->sip_hdr_start;
   }
   (void) pthread_mutex_unlock (&msg->sip_msg_mutex);
...
void sip_free_content (_sip_msg_t * sip_msg)
{
...
/* Free all resources */
void sip_destroy_msg (_sip_msg_t * _sip_msg)
{
   (void) sip_delete_start_line (_sip_msg);
   ...
   while (_sip_msg->sip_msg_req_res != NULL)
   {
      ...
      if (sip_msg_type_ptr->is_request)
      {
         sip_request_t *reqline;

         reqline = &sip_msg_type_ptr->U.sip_request;
...
/* Check if the message can be modified. */
boolean_t sip_ok_to_modify_message (_sip_msg_t * _sip_msg)
{
   if (_sip_msg == NULL)
      return (B_FALSE);
#ifdef	__solaris__
...
int sip_add_response_line (sip_msg_t sip_response, int response, char *response_code)
{
   ...
   if (!sip_ok_to_modify_message (_sip_response))
   {
      (void) pthread_mutex_unlock (&_sip_response->sip_msg_mutex);
      return (ENOTSUP);
   }
   header_size = strlen (SIP_VERSION) + SIP_SPACE +
      SIP_SIZE_OF_STATUS_CODE + SIP_SPACE + strlen (response_code) + strlen (SIP_CRLF);
   ...
   ret = sip_parse_first_line (_sip_response->sip_msg_start_line, &_sip_response->sip_msg_req_res);
   (void) pthread_mutex_unlock (&_sip_response->sip_msg_mutex);
   return (ret);
}
 ...
 */
sip_msg_t sip_create_response (sip_msg_t sip_request, int response, char *response_code, char *totag, char *mycontact)
{
   _sip_msg_t *new_msg;
    ...
    * Copy From header.
    */
   if (_sip_find_and_copy_header (_sip_request, new_msg, SIP_FROM, NULL))
   {
      goto error;
   }
   /*
   ...
   {
      if (_sip_find_and_copy_header (_sip_request, new_msg, SIP_TO, NULL))
      {
         goto error;
      }
   }
   else
   {
      ...
      if (tag_alloc)
         free (totag);
      if (_sip_find_and_copy_header (_sip_request, new_msg, SIP_TO, xtra_param))
      {
         free (xtra_param);
         goto error;
      }
    ...
    */
   if (sip_search_for_header (_sip_request, SIP_RECORD_ROUTE, NULL) != NULL)
   {
      if (_sip_find_and_copy_all_header (_sip_request, new_msg, SIP_RECORD_ROUTE) != 0)
      {
         goto error;
      }
   }
   if (mycontact != NULL)
   {
      if (sip_add_contact (new_msg, NULL, mycontact, B_FALSE, NULL) != 0)
      {
         goto error;
      }
   }
...
static char *sip_add_aquot_to_str (char *str, boolean_t * alloc)
{
   char *new_str;
   char *tmp = str;
   int size;
...
/* Add an empty header */
static int sip_add_empty_hdr (sip_msg_t sip_msg, char *hdr_name)
{
   _sip_header_t *new_header;
   int header_size;
   _sip_msg_t *_sip_msg;
   ...
   header_size = strlen (hdr_name) + SIP_SPACE + csize;
   ...
   (void) snprintf (new_header->sip_hdr_start, header_size + 1, "%s %c", hdr_name, SIP_HCOLON);
...
static int
sip_add_2strs_to_msg (sip_msg_t sip_msg, char *hdr_name, char *str1, boolean_t qstr1, char *str2, char *plist, char sep)
{
   _sip_header_t *new_header;
   int header_size;
   _sip_msg_t *_sip_msg;
   ...
   if (sip_msg == NULL || str1 == NULL || str2 == NULL ||
       (str1 != NULL && str1[0] == '\0') || (str2 != NULL && str2[0] == '\0'))
   {
      return (EINVAL);
   }
   _sip_msg = (_sip_msg_t *) sip_msg;
   ...
   if (plist == NULL)
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         strlen (str1) + csize + strlen (str2) + strlen (SIP_CRLF);
   }
   else
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         strlen (str1) + csize + strlen (str2) + csize + strlen (plist) + strlen (SIP_CRLF);
   }
   if (qstr1)
   ...
   if (plist == NULL)
   {
      if (qstr1)
      {
         (void) snprintf (new_header->sip_hdr_start,
                          header_size + 1, "%s %c \"%s\"%c%s%s", hdr_name, SIP_HCOLON, str1, sep, str2, SIP_CRLF);
      }
      else
      {
         (void) snprintf (new_header->sip_hdr_start,
   ...
   else
   {
      if (qstr1)
      {
         (void) snprintf (new_header->sip_hdr_start,
                          header_size + 1,
                          "%s %c \"%s\"%c%s%c%s%s", hdr_name, SIP_HCOLON, str1, sep, str2, SIP_SEMI, plist, SIP_CRLF);
      }
      else
      {
         (void) snprintf (new_header->sip_hdr_start,
                          header_size + 1, "%s %c %s%c%s%c%s%s",
                          hdr_name, SIP_HCOLON, str1, sep, str2, SIP_SEMI, plist, SIP_CRLF);
      }
   }
...
static int sip_add_str_to_msg (sip_msg_t sip_msg, char *hdr_name, char *str, char *plist, char param_sep)
{
   _sip_header_t *new_header;
   int header_size;
   _sip_msg_t *_sip_msg;
   ...
   if (plist == NULL)
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE + +strlen (str) + strlen (SIP_CRLF);
   }
   else
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         +strlen (str) + csize + strlen (plist) + strlen (SIP_CRLF);
   }
   ...
   if (plist == NULL)
   {
      (void) snprintf (new_header->sip_hdr_start, header_size + 1, "%s %c %s%s", hdr_name, SIP_HCOLON, str, SIP_CRLF);
   }
   else
   {
      (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                       "%s %c %s%c%s%s", hdr_name, SIP_HCOLON, str, param_sep, plist, SIP_CRLF);
   }
   _sip_add_header (_sip_msg, new_header, B_TRUE, B_FALSE, NULL);
   (void) pthread_mutex_unlock (&_sip_msg->sip_msg_mutex);
...
static char *sip_int_to_str (int i)
{
   int count;
   int t;
   int x;
...
static int sip_add_int_to_msg (sip_msg_t sip_msg, char *hdr_name, int i, char *plist)
{
   _sip_header_t *new_header;
   int header_size;
   _sip_msg_t *_sip_msg;
   ...
   if (plist == NULL)
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE + strlen (digit_str) + strlen (SIP_CRLF);
   }
   else
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         strlen (digit_str) + csize + strlen (plist) + strlen (SIP_CRLF);
   }
   ...
   {
      (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                       "%s %c %s%c%s%s", hdr_name, SIP_HCOLON, digit_str, SIP_SEMI, plist, SIP_CRLF);
   }
   free (digit_str);
   _sip_add_header (_sip_msg, new_header, B_TRUE, B_FALSE, NULL);
   (void) pthread_mutex_unlock (&_sip_msg->sip_msg_mutex);
...
static int sip_add_intstr_to_msg (sip_msg_t sip_msg, char *hdr_name, int i, char *s, char *plist)
{
   _sip_header_t *new_header;
   int header_size;
   _sip_msg_t *_sip_msg;
   ...
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         strlen (digit_str) + csize + strlen (s) + strlen (SIP_CRLF);
   }
   else
   {
      header_size = strlen (hdr_name) + SIP_SPACE + csize + SIP_SPACE +
         strlen (digit_str) + csize + strlen (s) + csize + strlen (plist) + strlen (SIP_CRLF);
   }
   ...
   {
      (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                       "%s %c %s %s%s", hdr_name, SIP_HCOLON, digit_str, s, SIP_CRLF);
   }
   else
   {
      (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                       "%s %c %s %s%c%s%s", hdr_name, SIP_HCOLON, digit_str, s, SIP_SEMI, plist, SIP_CRLF);
   }
   free (digit_str);
   _sip_add_header (_sip_msg, new_header, B_TRUE, B_FALSE, NULL);
   (void) pthread_mutex_unlock (&_sip_msg->sip_msg_mutex);
 ...
 */
int sip_add_accept (sip_msg_t sip_msg, char *type, char *subtype, char *m_par, char *a_par)
{
   int ret;
   char *plist;
   int size;
   ...
   if ((type != NULL) && (subtype != NULL))
   {
      ret = sip_add_2strs_to_msg (sip_msg, SIP_ACCEPT, type, B_FALSE, subtype, plist, SIP_SLASH);
   }
   else if (type != NULL)
   {
      ret = sip_add_2strs_to_msg (sip_msg, SIP_ACCEPT, type, B_FALSE, "*", plist, SIP_SLASH);
   }
   else
   {
      ret = EINVAL;
...
int sip_add_accept_enc (sip_msg_t sip_msg, char *code, char *plist)
{
   int ret;

   if (code == NULL)
   {
      ret = sip_add_str_to_msg (sip_msg, SIP_ACCEPT_ENCODE, "*", plist, SIP_SEMI);
   }
   else
   {
      ret = sip_add_str_to_msg (sip_msg, SIP_ACCEPT_ENCODE, code, plist, SIP_SEMI);
   }
   return (ret);
}
 ...
 *
 */
int sip_add_accept_lang (sip_msg_t sip_msg, char *lang, char *plist)
{
   int ret;
   ...
   ret = sip_add_str_to_msg (sip_msg, SIP_ACCEPT_LANG, lang, plist, SIP_SEMI);
   return (ret);
}
 ...
 *
 */
int sip_add_alert_info (sip_msg_t sip_msg, char *alert, char *plist)
{
   int ret;
   char *tmp;
   boolean_t alloc;
...
int sip_add_allow (sip_msg_t sip_msg, sip_method_t method)
{
   ...
   if (method == 0 || method >= MAX_SIP_METHODS)
      return (EINVAL);
   ret = sip_add_str_to_msg (sip_msg, SIP_ALLOW, sip_methods[method].name, NULL, '\0');
   return (ret);
}
 ...
 *
 */
int sip_add_call_info (sip_msg_t sip_msg, char *uri, char *plist)
{
   char *tmp;
   boolean_t alloc;
   int r;
 ...
 *
 */
int sip_add_content_disp (sip_msg_t sip_msg, char *dis_type, char *plist)
{
   int ret;
   ...
   ret = sip_add_str_to_msg (sip_msg, SIP_CONTENT_DIS, dis_type, plist, SIP_SEMI);
   return (ret);
}
 ...
 *
 */
int sip_add_error_info (sip_msg_t sip_msg, char *uri, char *plist)
{
   char *tmp;
   boolean_t alloc;
   int r;
...
int sip_add_org (sip_msg_t sip_msg, char *org)
{
   ...
   {
      ret = sip_add_str_to_msg (sip_msg, SIP_ORGANIZATION, org, NULL, '\0');
   }
   return (ret);
}
 ...
 *
 */
int sip_add_reply_to (sip_msg_t sip_msg, char *uname, char *addr, char *plist, boolean_t add_aquot)
{
   return (sip_add_name_aspec (sip_msg, uname, addr, NULL, add_aquot, SIP_REPLYTO, plist));
}
 ...
 *			/ generic-param
 */
int sip_add_retry_after (sip_msg_t sip_msg, int secs, char *cmt, char *plist)
{
   int r;
...
int sip_add_subject (sip_msg_t sip_msg, char *subject)
{
   ...
   {
      ret = sip_add_str_to_msg (sip_msg, SIP_SUBJECT, subject, NULL, '\0');
   }
   return (ret);
}
...
int sip_add_supported (sip_msg_t sip_msg, char *support)
{
   ...
   {
      ret = sip_add_str_to_msg (sip_msg, SIP_SUPPORT, support, NULL, '\0');
   }
   return (ret);
}
...
int sip_add_tstamp (sip_msg_t sip_msg, char *time, char *delay)
{
   ...
   {
      ret = sip_add_2strs_to_msg (sip_msg, SIP_TIMESTAMP, time, B_FALSE, delay, NULL, ' ');
   }
   return (ret);
}
...
int sip_add_warning (sip_msg_t sip_msg, int code, char *addr, char *msg)
{
   ...
   if (sip_msg == NULL || addr == NULL || msg == NULL || addr[0] == '\0' || msg == '\0' || code < 100 || code > 999)
   {
      return (EINVAL);
   }
   ...
   header_size = strlen (hdr_name) + SIP_SPACE + sizeof (char) + SIP_SPACE +
      sip_num_of_bytes (code) + SIP_SPACE + strlen (addr) + SIP_SPACE +
      sizeof (char) + strlen (msg) + sizeof (char) + strlen (SIP_CRLF);
   ...
   (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                    "%s %c %d %s \"%s\"%s", hdr_name, SIP_HCOLON, code, addr, msg, SIP_CRLF);
   _sip_add_header (_sip_msg, new_header, B_TRUE, B_FALSE, NULL);
   (void) pthread_mutex_unlock (&_sip_msg->sip_msg_mutex);
...
int sip_add_rack (sip_msg_t sip_msg, int resp_num, int cseq, sip_method_t method)
{
   ...
   if (sip_msg == NULL || resp_num <= 0 || cseq < 0 || method <= 0 || method >= MAX_SIP_METHODS)
   {
      return (EINVAL);
   }
   ...
   header_size = strlen (hdr_name) + SIP_SPACE + sizeof (char) + SIP_SPACE +
      sip_num_of_bytes (resp_num) + SIP_SPACE + sip_num_of_bytes (cseq) +
      SIP_SPACE + strlen (sip_methods[method].name) + strlen (SIP_CRLF);
   ...
   (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                    "%s %c %d %d %s%s", hdr_name, SIP_HCOLON, resp_num, cseq, sip_methods[method].name, SIP_CRLF);
 ...
 *			*(COMMA event-type)
 */
int sip_add_allow_events (sip_msg_t sip_msg, char *t_event)
{
   return (sip_add_str_to_msg (sip_msg, SIP_ALLOW_EVENTS, t_event, NULL, '\0'));
}
 ...
 */
int sip_add_event (sip_msg_t sip_msg, char *t_event, char *plist)
{
   return (sip_add_str_to_msg (sip_msg, SIP_EVENT, t_event, plist, SIP_SEMI));
}
 ...
 */
int sip_add_substate (sip_msg_t sip_msg, char *sub, char *plist)
{
   return (sip_add_str_to_msg (sip_msg, SIP_SUBSCRIPTION_STATE, sub, plist, SIP_SEMI));
}
 ...
 *
 */
int sip_add_proxy_authen (sip_msg_t sip_msg, char *pascheme, char *paparam)
{
   return (sip_add_str_to_msg (sip_msg, SIP_PROXY_AUTHEN, pascheme, paparam, SIP_SP));
}
 ...
 *
 */
int sip_add_proxy_author (sip_msg_t sip_msg, char *paschem, char *paparam)
{
   return (sip_add_str_to_msg (sip_msg, SIP_PROXY_AUTHOR, paschem, paparam, SIP_SP));
}
 ...
 *
 */
int sip_add_www_authen (sip_msg_t sip_msg, char *wascheme, char *waparam)
{
   return (sip_add_str_to_msg (sip_msg, SIP_WWW_AUTHEN, wascheme, waparam, SIP_SP));
}
...
int sip_add_cseq (sip_msg_t sip_msg, sip_method_t method, uint32_t cseq)
{
   ...
   if (sip_msg == NULL || (int) cseq < 0 || method == 0 || method >= MAX_SIP_METHODS)
   {
      return (EINVAL);
   }
   r = sip_add_intstr_to_msg (sip_msg, SIP_CSEQ, cseq, sip_methods[method].name, NULL);
   return (r);
}
 ...
 *
 */
_sip_header_t *sip_create_via_hdr (char *sent_protocol_transport, char *sent_by_host,
                                   int sent_by_port, char *via_params)
{
   ...
   header_size = strlen (SIP_VIA) + SIP_SPACE + sizeof (char) + SIP_SPACE +
      strlen (SIP_VERSION) + sizeof (char) +
      strlen (sent_protocol_transport) + SIP_SPACE + strlen (sent_by_host) + strlen (SIP_CRLF);

   if (sent_by_port > 0)
   {
      header_size += SIP_SPACE + sizeof (char) + SIP_SPACE + sip_num_of_bytes (sent_by_port);
   }

   if (via_params != NULL)
      header_size += SIP_SPACE + sizeof (char) + strlen (via_params);

   new_header = sip_new_header (header_size);
   if (new_header->sip_hdr_start == NULL)
      return (NULL);
   count = snprintf (new_header->sip_hdr_current, header_size + 1,
                     "%s %c %s/%s %s", SIP_VIA, SIP_HCOLON, SIP_VERSION, sent_protocol_transport, sent_by_host);
   new_header->sip_hdr_current += count;
   header_size -= count;

   if (sent_by_port > 0)
   {
      count = snprintf (new_header->sip_hdr_current, header_size + 1, " %c %d", SIP_HCOLON, sent_by_port);
      new_header->sip_hdr_current += count;
      header_size -= count;
   }
   ...
   if (via_params != NULL)
   {
      count = snprintf (new_header->sip_hdr_current, header_size + 1, " %c%s", SIP_SEMI, via_params);
      new_header->sip_hdr_current += count;
      header_size -= count;
   }

   (void) snprintf (new_header->sip_hdr_current, header_size + 1, "%s", SIP_CRLF);
   return (new_header);
}
 ...
 */
int
sip_add_via (sip_msg_t sip_msg, char *sent_protocol_transport, char *sent_by_host, int sent_by_port, char *via_params)
{
   _sip_header_t *new_header;
   _sip_msg_t *_sip_msg;

   if (sip_msg == NULL || sent_protocol_transport == NULL || sent_by_host == NULL || sent_by_port < 0)
   {
      return (EINVAL);
   }
   ...
   new_header = sip_create_via_hdr (sent_protocol_transport, sent_by_host, sent_by_port, via_params);
   if (new_header == NULL)
   {
      (void) pthread_mutex_unlock (&_sip_msg->sip_msg_mutex);
      return (ENOMEM);
...
int sip_add_maxforward (sip_msg_t sip_msg, uint_t maxforward)
{
   if (sip_msg == NULL || (int) maxforward < 0)
      return (EINVAL);
   return (sip_add_int_to_msg (sip_msg, SIP_MAX_FORWARDS, maxforward, NULL));
}
...
int sip_add_content (sip_msg_t sip_msg, char *content)
{
   ...
   (void) strncpy (msg_content->sip_content_start, content, strlen (content));
   msg_content->sip_content_start[strlen (content)] = '\0';
   msg_content->sip_content_current = msg_content->sip_content_start;
   msg_content->sip_content_end = msg_content->sip_content_start + strlen (msg_content->sip_content_start);
...
int sip_add_content_type (sip_msg_t sip_msg, char *type, char *subtype)
{
   if (sip_msg == NULL || type == NULL || subtype == NULL)
      return (EINVAL);
   return (sip_add_2strs_to_msg (sip_msg, SIP_CONTENT_TYPE, type, B_FALSE, subtype, NULL, SIP_SLASH));
}
 ...
 *
 */
int sip_add_content_length (_sip_msg_t * _sip_msg, int length)
{
   _sip_header_t *new_header;
   int header_size;
   ...
   header_size = strlen (SIP_CONTENT_LENGTH) + SIP_SPACE + sizeof (char) +
      SIP_SPACE + sip_num_of_bytes (length) + strlen (SIP_CRLF) + strlen (SIP_CRLF);
   ...
   (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                    "%s %c %u%s%s", SIP_CONTENT_LENGTH, SIP_HCOLON, length, SIP_CRLF, SIP_CRLF);
                    ...
                    char *tags, boolean_t add_aquot, char *header_name, char *params)
{
   ...
   {
      r = sip_add_2strs_to_msg (sip_msg, header_name, " ", B_FALSE, t, params, SIP_SP);
   }
   else
   {
      r = sip_add_2strs_to_msg (sip_msg, header_name, display_name, B_TRUE, t, params, SIP_SP);
   }
   if (qalloc)
      free (t);
   if (palloc)
 ...
 */
int
sip_add_contact (sip_msg_t sip_msg, char *display_name, char *contact_uri, boolean_t add_aquot, char *contact_params)
{
   return (sip_add_name_aspec (sip_msg, display_name, contact_uri, NULL, add_aquot, SIP_CONTACT, contact_params));
}
...
{
   return (sip_add_name_aspec (sip_msg, display_name, from_uri, fromtags, add_aquot, SIP_FROM, from_params));
}
 ...
 *
 */
int sip_add_to (sip_msg_t sip_msg, char *display_name, char *to_uri, char *totags, boolean_t add_aquot, char *to_params)
{
   return (sip_add_name_aspec (sip_msg, display_name, to_uri, totags, add_aquot, SIP_TO, to_params));
}
...
{
   return (sip_add_name_aspec (sip_msg, display_name, uri, NULL, B_TRUE, SIP_ROUTE, route_params));
}
...
{
   return (sip_add_name_aspec (sip_msg, display_name, uri, NULL, B_TRUE, SIP_RECORD_ROUTE, route_params));
}
 ...
 *
 */
int sip_add_passertedid (sip_msg_t sip_msg, char *display_name, char *addr, boolean_t add_aquot)
{
   return (sip_add_name_aspec (sip_msg, display_name, addr, NULL, add_aquot, SIP_PASSERTEDID, NULL));
}
...
{
   return (sip_add_name_aspec (sip_msg, display_name, addr, NULL, add_aquot, SIP_PPREFERREDID, NULL));
}
 ...
 * to "ACK".
 */
int sip_create_nonOKack (sip_msg_t request, sip_msg_t response, sip_msg_t ack_msg)
{
   int seqno;
   char *uri;
   _sip_msg_t *_request;
   ...
   {
      if ((ret = sip_parse_first_line (_request->sip_msg_start_line, &_request->sip_msg_req_res)) != 0)
      {
         (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
         return (ret);
      }
   }
   if (_request->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_ptr == NULL)
   {
      (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
      return (EINVAL);
   }
   uri = (char *) malloc (_request->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_len + 1);
   if (uri == NULL)
   {
      (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
      return (EINVAL);
   ...
   (void) strncpy (uri,
                   ...
                   sip_str_ptr, _request->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_len);
   uri[_request->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_len] = '\0';
   ...
   free (uri);
   if ((ret = _sip_find_and_copy_header (_request, _ack_msg, SIP_VIA, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
      return (ret);
   }
   (void) _sip_find_and_copy_header (_request, _ack_msg, SIP_MAX_FORWARDS, NULL);

   (void) pthread_mutex_lock (&_response->sip_msg_mutex);
   if ((ret = _sip_find_and_copy_header (_response, _ack_msg, SIP_TO, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (ret);
   }
   (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
   if ((ret = _sip_find_and_copy_header (_request, _ack_msg, SIP_FROM, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
      return (ret);
   }
   if ((ret = _sip_find_and_copy_header (_request, _ack_msg, SIP_CALL_ID, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_request->sip_msg_mutex);
      return (ret);
   }
                  ...
                  char *sent_by, int sent_by_port, char *via_params)
{
   ...
   (void) pthread_mutex_lock (&_response->sip_msg_mutex);
   if ((header = sip_search_for_header (_response, SIP_CONTACT, NULL)) == NULL)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (EINVAL);
   }
   if ((ret = sip_parse_cftr_header (header, (void *) &parsed_header)) != 0)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (ret);
   }
   ...
   (void) strncpy (uri, contact_value->cftr_uri.sip_str_ptr, contact_value->cftr_uri.sip_str_len);
   ...
   free (uri);
   if ((ret = sip_add_via (_ack_msg, transport, sent_by, sent_by_port, via_params)) != 0)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (ret);
   }
   ...
   if ((ret = _sip_find_and_copy_header (_response, _ack_msg, SIP_FROM, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (ret);
   }
   if ((ret = _sip_find_and_copy_header (_response, _ack_msg, SIP_CALL_ID, NULL)) != 0)
   {
      (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
      return (ret);
   }
   ...
   {
      if ((ret = _sip_find_and_copy_header (_response, _ack_msg, SIP_MAX_FORWARDS, NULL)) != 0)
      {
         (void) pthread_mutex_unlock (&_response->sip_msg_mutex);
         return (ret);
      }
...
int sip_add_request_line (sip_msg_t sip_request, sip_method_t method, char *request_uri)
{
   ...
   if (method < INVITE || method >= MAX_SIP_METHODS || request_uri == NULL || sip_request == NULL)
   {
      return (EINVAL);
   }
   ...
   _sip_request = (_sip_msg_t *) sip_request;
   (void) pthread_mutex_lock (&_sip_request->sip_msg_mutex);
   if (!sip_ok_to_modify_message (_sip_request))
   {
      (void) pthread_mutex_unlock (&_sip_request->sip_msg_mutex);
      return (ENOTSUP);
   }

   header_size = strlen (sip_methods[method].name) + SIP_SPACE +
      strlen (request_uri) + SIP_SPACE + strlen (SIP_VERSION) + strlen (SIP_CRLF);
   ...
   (void) snprintf (new_header->sip_hdr_start, header_size + 1,
                    "%s %s %s%s", sip_methods[method].name, request_uri, SIP_VERSION, SIP_CRLF);
   ...
   (void) sip_parse_first_line (_sip_request->sip_msg_start_line, &_sip_request->sip_msg_req_res);
   (void) pthread_mutex_unlock (&_sip_request->sip_msg_mutex);
   return (0);
}
...
sip_msg_t sip_clone_msg (sip_msg_t sip_msg)
{
   ...
   while (sip_content != NULL)
   {
      ...
      (void) strncpy (msg_content->sip_content_start, sip_content->sip_content_start, len);
      msg_content->sip_content_start[len] = '\0';
      msg_content->sip_content_current = msg_content->sip_content_start;
      msg_content->sip_content_end = msg_content->sip_content_start + len;
      msg_content->sip_content_allocated = B_TRUE;
...
char *sip_sent_by_to_str (int *error)
{
   ...
   for (cnt = 0; cnt < sip_sent_by_count; cnt++)
   {
      ...
      {
         count = snprintf (p, slen, "%c%s", SIP_COMMA, sb->sb_val);
      }
      p += count;
      slen -= count;
      sb = sb->sb_next;
 ...
 */
boolean_t sip_sent_by_registered (const sip_str_t * sb_val)
{
   sent_by_list_t *sb;
   int count = 0;
   ...
   {
      if (strncmp (sb->sb_val, sb_val->sip_str_ptr, sb_val->sip_str_len) == 0)
      {
         (void) pthread_mutex_unlock (&sip_sent_by_lock);
         return (B_TRUE);
      }
...
void sip_unregister_all_sent_by ()
{
   sent_by_list_t *sb;
   int count;
