...
sip_create_dialog_req (sip_method_t method, sip_dialog_t dialog,
                       char *transport, char *sent_by, int sent_by_port, char *via_param, uint32_t maxforward, int cseq)
{
   ...
   if (!sip_manage_dialog || dialog == NULL || transport == NULL || sent_by == NULL)
   {
      return (NULL);
   }
   ...
   if (sip_copy_header (sip_msg, _dialog->sip_dlg_local_uri_tag, NULL) != 0)
      goto err_ret;
   if (sip_copy_header (sip_msg, _dialog->sip_dlg_remote_uri_tag, NULL) != 0)
   {
      goto err_ret;
   }
   if (sip_copy_header (sip_msg, _dialog->sip_dlg_remote_target, NULL) != 0)
      goto err_ret;
   if (sip_add_via (sip_msg, transport, sent_by, sent_by_port, via_param) != 0)
   {
      goto err_ret;
   }
   ...
   {
      if (sip_copy_header (sip_msg, _dialog->sip_dlg_route_set, NULL) != 0)
      {
         _dialog->sip_dlg_local_cseq = oldseq;
         goto err_ret;
      }
...
const sip_str_t *sip_get_dialog_local_tag (sip_dialog_t dialog, int *error)
{
   ...
   {
      val = sip_get_header_value (_dialog->sip_dlg_local_uri_tag, error);
      if (val == NULL)
      {
         (void) pthread_mutex_unlock (&_dialog->sip_dlg_mutex);
         return (NULL);
      }
      ltag = sip_get_param_value ((sip_header_value_t) val, "tag", error);
   }
   (void) pthread_mutex_unlock (&_dialog->sip_dlg_mutex);
   return (ltag);
}
...
const sip_str_t *sip_get_dialog_remote_tag (sip_dialog_t dialog, int *error)
{
   ...
   {
      val = sip_get_header_value (_dialog->sip_dlg_remote_uri_tag, error);
      if (val == NULL)
      {
         (void) pthread_mutex_unlock (&_dialog->sip_dlg_mutex);
         return (NULL);
      }
      ttag = sip_get_param_value ((sip_header_value_t) val, "tag", error);
   }
   (void) pthread_mutex_unlock (&_dialog->sip_dlg_mutex);
...
const struct sip_uri *sip_get_dialog_remote_target_uri (sip_dialog_t dialog, int *error)
{
   ...
   {
      val = sip_get_header_value (_dialog->sip_dlg_remote_target, error);
      if (val == NULL)
      {
         (void) pthread_mutex_unlock (&_dialog->sip_dlg_mutex);
         return (NULL);
...
/* Partial dialog ? */
boolean_t sip_incomplete_dialog (sip_dialog_t dialog)
{
   _sip_dialog_t *_dialog;
   boolean_t isnew;
