...
int sip_find_md5_digest (char *bid, _sip_msg_t * msg, uint16_t * hindex, sip_method_t method)
{
   boolean_t is_2543;

   is_2543 = (bid == NULL || strncmp (bid, RFC_3261_BRANCH, strlen (RFC_3261_BRANCH)) != 0);
   ...
   if (is_2543)
   {
      ...
      sip_md5_hash (via->sip_hdr_start,
                    ...
                    to_uri->sip_str_ptr, to_uri->sip_str_len, (uchar_t *) hindex);
   }
   else
   {
      sip_md5_hash (bid, strlen (bid), (char *) &method,
                    sizeof (sip_method_t), NULL, 0, NULL, 0, NULL, 0, NULL, 0, (uchar_t *) hindex);
   }
   return (0);
}

/* Add object to the connection cache object. Not checking for duplicates!! */
int sip_add_conn_obj_cache (sip_conn_object_t obj, void *cobj)
{
   void **obj_val;
   ...
   {
      if (sip_is_conn_obj_cache (sip_trans->sip_xaction_conn_obj, (void *) sip_trans))
      {
         return (0);
      }
 ...
 */
static boolean_t sip_is_conn_obj_cache (sip_conn_object_t obj, void *cobj)
{
   void **obj_val;
   sip_conn_obj_pvt_t *pvt_data;
   sip_conn_cache_t *xaction_list;
 ...
 * refrele the objs.
 */
void sip_del_conn_obj_cache (sip_conn_object_t obj, void *cobj)
{
   void **obj_val;
   sip_conn_obj_pvt_t *pvt_data;
   sip_conn_cache_t *xaction_list;
 ...
 */
boolean_t sip_xaction_match (void *obj, void *hindex)
{
   ...
   if (SIP_IS_XACTION_TERMINATED (tmp->sip_xaction_state))
      return (B_FALSE);
   if (bcmp (tmp->sip_xaction_hash_digest, hindex, sizeof (tmp->sip_xaction_hash_digest)) == 0)
   {
...
/* Find a transaction */
static sip_xaction_t *sip_xaction_find (char *branchid, _sip_msg_t * msg, int which)
{
   sip_xaction_t *tmp;
    ...
    */
   if (sip_msg_info->is_request && which == SIP_SERVER_TRANSACTION && (method == ACK || method == CANCEL))
   {
      method = INVITE;
   }
   ...
   tmp = (sip_xaction_t *) sip_hash_find (sip_xaction_hash, (void *) hash_index, hindex, sip_xaction_match);
   return (tmp);
}
 ...
 */
static sip_xaction_t *sip_xaction_create (sip_conn_object_t obj, _sip_msg_t * msg, char *branchid, int *error)
{
   sip_xaction_t *trans;
   sip_message_type_t *sip_msg_info;
   int state = 0;
   ...
   {
      trans->sip_xaction_branch_id = (char *) malloc (strlen (branchid) + 1);
      if (trans->sip_xaction_branch_id == NULL)
      {
         free (trans);
         if (error != NULL)
      ...
      (void) strncpy (trans->sip_xaction_branch_id, branchid, strlen (branchid));
   ...
   else
   {
      ...
      if (method == INVITE)
         state = SIPS_SRV_INV_PROCEEDING;
      else
         state = SIPS_SRV_TRYING;
   }
   ...
   SIP_INIT_TIMER (trans->sip_xaction_TB, 64 * timer1) SIP_INIT_TIMER (trans->sip_xaction_TD, timerd);
   ...
   if (sip_xaction_ulp_state_cb != NULL && prev_state != trans->sip_xaction_state)
   {
      sip_xaction_ulp_state_cb ((sip_transaction_t) trans, (sip_msg_t) msg, prev_state, trans->sip_xaction_state);
   }
   return (trans);
}

/* Find a transaction, create if asked for */
sip_xaction_t *sip_xaction_get (sip_conn_object_t obj, sip_msg_t msg, boolean_t create, int which, int *error)
{
   char *branchid;
   ...
   if (sip_trans == NULL && create)
   {
       ...
       */
      if (sip_msg_info->is_request &&
          (branchid == NULL || strncmp (branchid, RFC_3261_BRANCH, strlen (RFC_3261_BRANCH) != 0)))
      {
         if (error != NULL)
            *error = EINVAL;
         if (branchid != NULL)
 ...
 * sip_hash_delete().
 */
boolean_t sip_xaction_remove (void *obj, void *hindex, int *found)
{
   sip_xaction_t *tmp = (sip_xaction_t *) obj;
   ...
   *found = 0;
   ...
   if (bcmp (tmp->sip_xaction_hash_digest, hindex, sizeof (tmp->sip_xaction_hash_digest)) == 0)
   {
      *found = 1;
      ...
      {
         sip_del_conn_obj_cache (tmp->sip_xaction_conn_obj, (void *) tmp);
      }
      free (tmp);
      return (B_TRUE);
   }
 ...
 */
void sip_xaction_delete (sip_xaction_t * trans)
{
   int hindex;

   hindex = SIP_DIGEST_TO_HASH (trans->sip_xaction_hash_digest);

   sip_hash_delete (sip_xaction_hash, trans->sip_xaction_hash_digest, hindex, sip_xaction_remove);
}

/*
 * Add a SIP transaction into the hash list.
 */
int sip_xaction_add (sip_xaction_t * trans, char *branchid, _sip_msg_t * msg, sip_method_t method)
{
   uint16_t hash_index[8];
   ...
   if (sip_hash_add (sip_xaction_hash, (void *) trans, SIP_DIGEST_TO_HASH (hash_index)) != 0)
   {
      return (ENOMEM);
   }
   return (0);
...
char *sip_get_xaction_state (int state)
{
   switch (state)
   {
   case SIPS_CLNT_CALLING:
      return ("SIPS_CLNT_CALLING");
   case SIPS_CLNT_INV_PROCEEDING:
      return ("SIPS_CLNT_INV_PROCEEDING");
   case SIPS_CLNT_INV_TERMINATED:
      return ("SIPS_CLNT_INV_TERMINATED");
   case SIPS_CLNT_INV_COMPLETED:
      return ("SIPS_CLNT_INV_COMPLETED");
   case SIPS_CLNT_TRYING:
      return ("SIPS_CLNT_TRYING");
   case SIPS_CLNT_NONINV_PROCEEDING:
      return ("SIPS_CLNT_NONINV_PROCEEDING");
   case SIPS_CLNT_NONINV_TERMINATED:
      return ("SIPS_CLNT_NONINV_TERMINATED");
   case SIPS_CLNT_NONINV_COMPLETED:
      return ("SIPS_CLNT_NONINV_COMPLETED");
   case SIPS_SRV_INV_PROCEEDING:
      return ("SIPS_SRV_INV_PROCEEDING");
   case SIPS_SRV_INV_COMPLETED:
      return ("SIPS_SRV_INV_COMPLETED");
   case SIPS_SRV_CONFIRMED:
      return ("SIPS_SRV_CONFIRMED");
   case SIPS_SRV_INV_TERMINATED:
      return ("SIPS_SRV_INV_TERMINATED");
   case SIPS_SRV_TRYING:
      return ("SIPS_SRV_TRYING");
   case SIPS_SRV_NONINV_PROCEEDING:
      return ("SIPS_SRV_NONINV_PROCEEDING");
   case SIPS_SRV_NONINV_COMPLETED:
      return ("SIPS_SRV_NONINV_COMPLETED");
   case SIPS_SRV_NONINV_TERMINATED:
      return ("SIPS_SRV_NONINV_TERMINATED");
   default:
      return ("unknown");
   }
}
                  ...
                  void (*ulp_state_cb) (sip_transaction_t, sip_msg_t, int, int))
{
   ...
   for (cnt = 0; cnt < SIP_HASH_SZ; cnt++)
   {
      ...
      (void) pthread_mutex_init (&sip_xaction_hash[cnt].sip_hash_mutex, NULL);
   }
