...
#define	MAX_SIP_HEADERS 54

/* Built-In Header function table */

sip_header_function_t sip_header_function_table[MAX_SIP_HEADERS] = {
   {"Unknown", NULL, sip_parse_unknown_header, NULL, NULL, NULL},
   {"CONTACT", "m", sip_parse_cftr_header, NULL, NULL, sip_free_cftr_header},
   {"FROM", "F", sip_parse_cftr_header, NULL, NULL, sip_free_cftr_header},
   {"TO", "T", sip_parse_cftr_header, NULL, NULL, sip_free_cftr_header},
   {"CONTENT-LENGTH", "l", sip_parse_clen_header, NULL, NULL, sip_free_phdr},
   {"CONTENT-TYPE", "c", sip_parse_ctype_header, NULL, NULL, sip_free_phdr},
   ...
   {"Max-Forwards", NULL, sip_parse_maxf_header, NULL, NULL, sip_free_phdr},
   {"RECORD-ROUTE", NULL, sip_parse_cftr_header, NULL, NULL, sip_free_phdr},
   {"ROUTE", NULL, sip_parse_cftr_header, NULL, NULL, sip_free_phdr},
   ...
   {"CALL-INFO", NULL, sip_parse_callinfo_header, NULL, NULL, sip_free_phdr},
   ...
   {"ERROR-INFO", NULL, sip_parse_errorinfo_header, NULL, NULL, sip_free_phdr},
   {"EXPIRES", NULL, sip_parse_expire_header, NULL, NULL, sip_free_phdr},
   {"IN-REPLY-TO", NULL, sip_parse_inreplyto_header, NULL, NULL, sip_free_phdr},
   {"MIN-EXPIRES", NULL, sip_parse_minexpire_header, NULL, NULL, sip_free_phdr},
   {"MIME-VERSION", NULL, sip_parse_mimeversion_header, NULL, NULL, sip_free_phdr},
   {"ORGANIZATION", NULL, sip_parse_org_header, NULL, NULL, sip_free_phdr},
   {"PRIORITY", NULL, sip_parse_priority_header, NULL, NULL, sip_free_phdr},
   ...
   {"RETRY-AFTER", NULL, sip_parse_retryaft_header, NULL, NULL, sip_free_phdr},
   {"SERVER", NULL, sip_parse_server_header, NULL, NULL, sip_free_phdr},
   {"SUBJECT", "s", sip_parse_subject_header, NULL, NULL, sip_free_phdr},
   {"TIMESTAMP", NULL, sip_parse_timestamp_header, NULL, NULL, sip_free_phdr},
   {"UNSUPPORTED", NULL, sip_parse_usupport_header, NULL, NULL, sip_free_phdr},
   {"SUPPORTED", "k", sip_parse_support_header, NULL, NULL, sip_free_phdr},
   {"USER-AGENT", NULL, sip_parse_useragt_header, NULL, NULL, sip_free_phdr},
   {"WARNING", NULL, sip_parse_warn_header, NULL, NULL, sip_free_phdr},
   {"ALLOW-EVENTS", "u", sip_parse_allow_events_header, NULL, NULL, sip_free_phdr},
   {"EVENT", "o", sip_parse_event_header, NULL, NULL, sip_free_phdr},
   {"SUBSCRIPTION-STATE", NULL, sip_parse_substate_header, NULL, NULL,
    sip_free_phdr},
   {"AUTHORIZATION", NULL, sip_parse_author_header, NULL, NULL, sip_free_phdr},
   ...
   {"PROXY-AUTHENTICATE", NULL, sip_parse_pauthen_header, NULL, NULL,
    sip_free_phdr},
   {"PROXY-REQUIRE", NULL, sip_parse_preq_header, NULL, NULL, sip_free_phdr},
   {"WWW-AUTHENTICATE", NULL, sip_parse_wauthen_header, NULL, NULL, sip_free_phdr},
   {"RSEQ", NULL, sip_parse_rseq, NULL, NULL, sip_free_phdr},
   {"RACK", NULL, sip_parse_rack, NULL, NULL, sip_free_phdr},
   {"P-ASSERTED-IDENTITY", NULL, sip_parse_passertedid, NULL, NULL, sip_free_phdr},
...
/* Free parameter list */
static void sip_free_params (sip_param_t * param_list)
{
...
void sip_free_phdr (sip_parsed_header_t * header)
{
...
void sip_free_cftr_header (sip_parsed_header_t * header)
{
   sip_hdr_value_t *value;
   sip_hdr_value_t *next_value;
...
/* Return new header */
_sip_header_t *sip_new_header (int header_size)
{
...
void sip_free_header (_sip_header_t * sip_header)
{
   ...
   if (sip_header->sip_hdr_parsed != NULL)
   {
      ...
      {
         sip_header->sip_header_functions->header_free (sip_header->sip_hdr_parsed);
      }
   }
   free (sip_header);
}
...
_sip_header_t *sip_dup_header (_sip_header_t * from)
{
   size_t hdr_size;
   _sip_header_t *to;
   ...
   if (from->sip_header_state == SIP_HEADER_DELETED_VAL)
   {
      ...
      len = sip_copy_values (to->sip_hdr_start, from);
      to->sip_hdr_end = to->sip_hdr_start + len;
   }
   else
   {
...
int _sip_copy_header (_sip_msg_t * sip_msg, _sip_header_t * header, char *extra_param, boolean_t skip_crlf)
{
   _sip_header_t *new_header;
   int hdrlen;
   int extra_len = 0;
   ...
   if (extra_param != NULL)
   {
      extra_len = SIP_SPACE + sizeof (char) + SIP_SPACE + strlen (extra_param);
   }
   /*
   ...
   if (skip_crlf)
   {
      if (header->sip_hdr_end - strlen (SIP_CRLF) <= header->sip_hdr_start)
      {
         goto proceed;
      }
 ...
 proceed:
   ...
   {
      (void) memcpy (new_header->sip_hdr_start, header->sip_hdr_start, hdrlen);
   ...
   if (extra_param != NULL)
   {
      ...
      (void) snprintf (new_header->sip_hdr_current, hdrlen + 1, " %c %s%s", SIP_SEMI, extra_param, SIP_CRLF);
   }
...
int _sip_find_and_copy_all_header (_sip_msg_t * _old_msg, _sip_msg_t * _new_msg, char *header_name)
{
...
/* Copy header_name from _old_msg to _new_msg with extra_parm, if any */
int _sip_find_and_copy_header (sip_msg_t _old_msg, sip_msg_t _new_msg, char *header_name, char *extra_param)
{
   _sip_header_t *header;
   int ret;
...
int sip_copy_all_headers (sip_msg_t old_msg, sip_msg_t new_msg)
{
   _sip_header_t *header;
   _sip_msg_t *_old_msg;
   _sip_msg_t *_new_msg;
...
int sip_copy_start_line (sip_msg_t msg, sip_msg_t sip_msg)
{
   int len;
   _sip_header_t *new_header;
   _sip_msg_t *_old_msg;
   ...
   len = _old_msg->sip_msg_start_line->sip_hdr_end - _old_msg->sip_msg_start_line->sip_hdr_start;
   ...
   (void) strncpy (_sip_msg->sip_msg_start_line->sip_hdr_start, _old_msg->sip_msg_start_line->sip_hdr_start, len);
   (void) sip_parse_first_line (_sip_msg->sip_msg_start_line, &_sip_msg->sip_msg_req_res);
   (void) pthread_mutex_unlock (&_old_msg->sip_msg_mutex);
...
void sip_delete_all_headers (_sip_msg_t * _sip_msg)
{
   _sip_header_t *header;
 ...
 * free all headers.
 */
void sip_delete_headers (sip_msg_t sip_msg, char *header_name)
{
   _sip_header_t *header;
   _sip_msg_t *_sip_msg;
   ...
   while (header != NULL)
   {
      ...
      {
         header->sip_hdr_prev->sip_hdr_next = header->sip_hdr_next;
      }
      ...
      {
         header->sip_hdr_next->sip_hdr_prev = header->sip_hdr_prev;
      }
      sip_free_header (header);
      if (header_name != NULL)
         return;
 ...
 */
void
_sip_add_header (_sip_msg_t * sip_msg, _sip_header_t * new_header, boolean_t append, boolean_t first, char *header_name)
{
   ...
   if (header_name != NULL)
   {
      ...
      if (!first)
      {
         ...
         {
            header_tmp = header;
            header = sip_search_for_header (sip_msg, header_name, header);
         }
      }
      header = header_tmp;
      if (header == NULL)
   ...
   else
   {
      if (append)
      {
         ...
         {
            sip_msg->sip_msg_headers_end->sip_hdr_next = new_header;
         }
         else
         {
            sip_msg->sip_msg_headers_start = new_header;
         }
         new_header->sip_hdr_prev = sip_msg->sip_msg_headers_end;
         new_header->sip_hdr_next = NULL;
         sip_msg->sip_msg_headers_end = new_header;
      }
      else
      {
         ...
         {
            sip_msg->sip_msg_headers_start->sip_hdr_prev = new_header;
         }
         else
         {
            sip_msg->sip_msg_headers_end = new_header;
         }
         new_header->sip_hdr_next = sip_msg->sip_msg_headers_start;
         new_header->sip_hdr_prev = NULL;
         sip_msg->sip_msg_headers_start = new_header;
      }
   }
   sip_msg->sip_msg_len += new_header->sip_hdr_end - new_header->sip_hdr_start;
}
 ...
 * type.
 */
sip_header_function_t *_sip_get_header_functions (sip_header_function_t * sip_header_function_table,
                                                  _sip_header_t * sip_header, char *header_name)
{
   ...
   if (header_name == NULL)
   {
      ...
      if (sip_find_separator (sip_header, SIP_HCOLON, '\0', '\0') != 0)
      {
         return (NULL);
      }
   ...
   if (len > 0)
   {
      while (sip_header_function_table[i].header_name != NULL || sip_header_function_table[i].header_short_name != NULL)
      {
         if (sip_header_function_table[i].header_name != NULL &&
             len == strlen (sip_header_function_table[i].header_name))
         {
            if (strncasecmp (header_name, sip_header_function_table[i].header_name, len) == 0)
            {
               break;
            }
         }
         else if (sip_header_function_table[i].
                  header_short_name != NULL && len == strlen (sip_header_function_table[i].header_short_name))
         {
            if (strncasecmp (header_name, sip_header_function_table[i].header_short_name, len) == 0)
            {
               break;
            }
         }
...
sip_header_function_t *sip_get_header_functions (_sip_header_t * sip_header, char *header_name)
{
   ...
   {
      header_f_table = _sip_get_header_functions (sip_header_function_table_external, sip_header, header_name);
      if (header_f_table != NULL)
         return (header_f_table);
   }
   func = _sip_get_header_functions (sip_header_function_table, sip_header, header_name);
   return (func);
}

/* Search for the header name passed in. */
_sip_header_t *sip_search_for_header (_sip_msg_t * sip_msg, char *header_name, _sip_header_t * old_header)
{
   int len = 0;
   int full_len = 0;
   int compact_len = 0;
   ...
   while (header != NULL)
   {
      ...
      if (full_name != NULL && (full_len <= len) && strncasecmp (header->sip_hdr_current, full_name, full_len) == 0)
      {
      ...
      if (compact_name != NULL && (compact_len <= len) &&
          strncasecmp (header->sip_hdr_current, compact_name, compact_len) == 0)
      {
   ...
   if (header != NULL)
   {
      ...
      if (header_f_table == NULL)
      {
         header_f_table = sip_get_header_functions (header, header_name);
         if (header_f_table == NULL)
            header_f_table = &sip_header_function_table[0];
      }
...
char *_sip_startline_to_str (_sip_msg_t * sip_msg, int *error)
{
   char *slstr;
   int len;
   ...
   (void) pthread_mutex_lock (&sip_msg->sip_msg_mutex);
   len = sip_msg->sip_msg_start_line->sip_hdr_end - sip_msg->sip_msg_start_line->sip_hdr_start - 2;
 ...
 * content.
 */
char *sip_msg_to_msgbuf (_sip_msg_t * msg, int *error)
{
   _sip_header_t *header;
   int len = 0;
   char *p;
   ...
   {
      len = msg->sip_msg_start_line->sip_hdr_end - msg->sip_msg_start_line->sip_hdr_start;
   ...
   while (header != NULL)
   {
      if (header->sip_header_state != SIP_HEADER_DELETED)
      {
         ...
         {
            len = header->sip_hdr_end - header->sip_hdr_start;
 ...
 * Retruns a pointer to the value or NULL.
 */
sip_param_t *sip_get_param_from_list (sip_param_t * param_list, char *param_name)
{
   while (param_list != NULL)
   {
      if (param_list->param_name.sip_str_len == strlen (param_name) &&
          strncasecmp (param_list->param_name.sip_str_ptr, param_name, strlen (param_name)) == 0)
      {
         return (param_list);
      }
