 ...
 */
static int SIP_URI_HEXVAL (int c)
{
   if (c >= 0x30 && c <= 0x39)
      return (c - '0');
   if (c >= 0x41 && c <= 0x46)
 ...
 */
static int sip_uri_url_casecmp (const char *str1, const char *str2, unsigned len)
{
   unsigned j;

   for (j = 0; j < len && tolower (str1[j]) == tolower (str2[j]) && str1[j] != '\0'; ++j)
   {
      ;
   }
 ...
 */
static int sip_uri_parse_tel (char *scan, char *uend)
{
   char *mark = (char *) 0;
   int ret = 0;
   int isGlobal = 0;
   ...
   else
   {
      while (scan < uend &&
             (SIP_URI_ISPHONEDIGIT (*scan) ||
              SIP_URI_ISDTMFDIGIT (*scan) || sip_uri_isEscapedPound (&scan, uend) || *scan == 'p' || *scan == 'w'))
      {
         ++scan;
      }
   }
   ...
   if (!isGlobal)
   {
      ...
      if (uend - scan > 15 && !sip_uri_url_casecmp (scan, ";phone-context=", 15))
      {
         scan += 15;
         mark = scan;
   ...
   while (scan < uend && ret)
   {
      ...
      else if (uend - scan > 5 && !sip_uri_url_casecmp (scan, ";tsp=", 5))
      {
         scan += 5;
         mark = scan;
         while (scan < uend && *scan != ';')
 ...
 * dtmf-digit            = "*" / "#" / "A" / "B" / "C" / "D"
 */
static int sip_uri_parse_tel_areaspe (char *scan, char *uend)
{
   int uri_hexValue;
   ...
   if (*scan == '+')
   {
      ...
      /* parse local-network-prefix */
            ...
            sip_uri_isEscapedPound (&scan, uend) || *scan == 'p' || *scan == 'w')
   {
      ++scan;
      while (scan < uend &&
             (SIP_URI_ISPHONEDIGIT (*scan) ||
   ...
   else
   {
      ...
      if (*scan == '!' || *scan == '$' || *scan == '&' ||
          ...
          *scan == '=' || *scan == '?' || *scan == '_' ||
          (*scan >= 'E' && *scan <= 'Z' &&
           *scan != 'P' && *scan != 'W') || (*scan >= 'e' && *scan <= 'z' && *scan != 'p' && *scan != 'w'))
      {
         ++scan;
      }
      else
      {
         ...
         if (uri_hexValue == 0x21 || uri_hexValue == 0x22 ||
             ...
             (uri_hexValue >= 0x71 && uri_hexValue <= 0x76) || (uri_hexValue >= 0x78 && uri_hexValue <= 0x7e))
         {
            scan += 3;
         }
         else
      ...
      while (scan < uend)
      {
         if (SIP_URI_ISUNRESERVED (*scan) || (SIP_URI_ISUSER (*scan) && *scan != ';'))
         {
            ++scan;
         }
         else
         {
            uri_hexValue = sip_uri_hexVal (scan, uend);
            if (uri_hexValue >= 0x21 && uri_hexValue <= 0x7e && uri_hexValue != 0x3b)
            {
               scan += 3;
            }
            else
...
static int sip_uri_hexVal (char *scan, char *uend)
{
   int ret = -1;
   ...
   if (SIP_URI_ISURLESCAPE (scan, uend))
   {
      ret = (SIP_URI_ISDIGIT (scan[1]) ? (scan[1] - '0') :
         ...
         (SIP_URI_ISDIGIT (scan[2]) ? (scan[2] - '0') : (tolower (scan[2]) - 'a' + 10));
   }
   return (ret);
}
 ...
 */
static int sip_uri_parse_tel_servicepro (char *scan, char *uend)
{
   char *mark = (char *) 0;
   ...
   while (scan < uend)
   {
      ...
      while (scan < uend && (*scan == '-' || SIP_URI_ISALNUM (*scan)))
         ++scan;
      if ((scan < uend && *scan != '.') || !SIP_URI_ISALPHA (*mark) || !SIP_URI_ISALNUM (*(scan - 1)))
      {
         return (0);
      }
      if (scan < uend)
 ...
 */
static int sip_uri_parse_tel_futureext (char *scan, char *uend)
{
   char *mark;
   int uri_hexValue = 0;
   ...
   while (scan < uend && sip_uri_isTokenchar (&scan, uend))
      ;
   if (mark == scan || (scan < uend && (*scan != '=' || scan + 1 == uend)))
   {
      return (0);
   }
   if (scan == uend)
   ...
   {                            /* parse quoted-string */
      ...
      while (scan < uend && sip_uri_hexVal (scan, uend) != 0x22)
      {
         ...
         if (sip_uri_hexVal (scan, uend) == 0x5c)
         {
            ...
            if (scan < uend)
            {
               if (SIP_URI_ISUNRESERVED (*scan) || SIP_URI_ISUSER (*scan))
               {
                  ++scan;
               }
               else if (sip_uri_hexVal (scan, uend) >= 0x00 && sip_uri_hexVal (scan, uend) <= 0x7f)
               {
                  scan += 3;
               }
               else
         ...
         else
         {
            ...
            else
            {
               ...
               if ((uri_hexValue >= 0x20 &&
                    uri_hexValue <= 0x21) ||
                   (uri_hexValue >= 0x23 && uri_hexValue <= 0x7e) || (uri_hexValue >= 0x80 && uri_hexValue <= 0xff))
               {
                  scan += 3;
               }
               else
      ...
      if (scan == uend || (scan < uend && sip_uri_hexVal (scan, uend) != 0x22))
      {
         return (0);
      }
      scan += 3;
 ...
 */
static int sip_uri_isTokenchar (char **pscan, char *uend)
{
   char *scan = *pscan;
   int uri_hexValue = 0;
   ...
   /* for ALPAH DIGIT - _ . ! ~ * ' $ & + */
   if ((SIP_URI_ISUNRESERVED (*scan) && *scan != '(' && *scan != ')') || *scan == '$' || *scan == '&' || *scan == '+')
   {
      ++scan;
      *pscan = scan;
      return (1);
   ...
   if (uri_hexValue == 0x21 || uri_hexValue == 0x7c ||
       ...
       (uri_hexValue >= 0x41 && uri_hexValue <= 0x5a) || (uri_hexValue >= 0x5e && uri_hexValue <= 0x7a))
   {
      scan += 3;
      *pscan = scan;
      return (1);
 ...
 * it must be escaped
 */
static int sip_uri_isEscapedPound (char **pscan, char *uend)
{
   char *scan = *pscan;

   if (scan == uend)
      return (0);
   if (*scan == '%' && scan + 2 < uend && scan[1] == '2' && scan[2] == '3')
   {
      scan += 2;
      *pscan = scan;
      return (1);
 ...
 */
static int sip_uri_parse_scheme (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend)
   {
 ...
 * param-unreserved	=  "[" / "]" / "/" / ":" / "&" / "+" / "$"
 */
static void sip_uri_parse_params (_sip_uri_t * outurl, char *scan, char *uend)
{
   char *mark = (char *) 0;
   char *equal = (char *) 0;
   int i = 0;
   ...
   while (scan < uend)
   {
      ...
      if (equal == (char *) 0)
      {
         ...
         while (mark < scan && (SIP_URI_ISPARAM (*mark) || SIP_URI_ISURLESCAPE (mark, scan)))
         {
            ++mark;
         }
      }
      else
      {
         ...
         if ((paramleftlen == 10 &&
             ...
             (paramleftlen == 7 && !sip_uri_url_casecmp (mark, "method=", 7)))
         {
            if (scan - equal == 1)
            {
               outurl->sip_uri_errflags |= SIP_URIERR_PARAM;
         ...
         else if (paramleftlen == 4 && !sip_uri_url_casecmp (mark, "ttl=", 4))
         {
            if (scan - equal == 1)
            {
               outurl->sip_uri_errflags |= SIP_URIERR_PARAM;
         ...
         else if (paramleftlen == 6 && !sip_uri_url_casecmp (mark, "maddr=", 6))
         {
            gothost = 0;
            mark = equal + 1;
            if (mark < scan && SIP_URI_ISDIGIT (*mark))
            {
               gothost = sip_uri_parse_ipv4 (mark, scan);
            }
            /*
            ...
            {
               gothost = sip_uri_parse_ipv6 (mark, scan);
            }
            /*
            ...
            {
               if (!(gothost = sip_uri_parse_hostname (mark, scan)))
               {
                  outurl->sip_uri_errflags |= SIP_URIERR_PARAM;
               }
            }
         ...
         else if (paramleftlen == 3 && !sip_uri_url_casecmp (mark, "lr=", 3))
         {
            outurl->sip_uri_errflags |= SIP_URIERR_PARAM;
            return;
         }
         ...
         {
            while (mark < scan && (SIP_URI_ISPARAM (*mark) || SIP_URI_ISURLESCAPE (mark, scan) || mark == equal))
            {
               ++mark;
            }
         }
 ...
 * hnv-unreserved  =  "[" / "]" / "/" / "?" / ":" / "+" / "$"
 */
static void sip_uri_parse_headers (_sip_uri_t * outurl, char *scan, char *uend)
{
   char *mark = NULL;
   char *equal = NULL;
   ...
   while (scan < uend)
   {
      ...
      while (mark < scan && (SIP_URI_ISHEADER (*mark) || SIP_URI_ISURLESCAPE (mark, scan) || mark == equal))
      {
         ++mark;
      }
      if (mark < scan)
 ...
 *                  / "&" / "=" / "+" / "$" / ","
 */
static void sip_uri_parse_abs_opaque (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend)
   {
   ...
   if (SIP_URI_ISUNRESERVED (*scan) || SIP_URI_ISURLESCAPE (scan, uend) ||
       SIP_URI_ISOTHER (*scan) || *scan == ';' || *scan == '?' || *scan == ':' || *scan == '@' || *scan == '&')
   {
      ++scan;
   }
   else
 ...
 * uric  =  reserved / unreserved / escaped
 */
static void sip_uri_parse_abs_query (_sip_uri_t * outurl, char *scan, char *uend)
{
 ...
 *                   ":" / "@" / "&" / "=" / "+" / "$" / ","
 */
static void sip_uri_parse_abs_path (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend || *scan != '/')
      return;
 ...
 *             / ":" / "@" / "&" / "=" / "+" )
 */
static void sip_uri_parse_abs_regname (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend)
      return;
 ...
 * password =  *( unreserved / escaped / "&" / "=" / "+" / "$" / "," )
 */
static void sip_uri_parse_password (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend || *scan != ':' || scan + 1 == uend)
      return;
   ++scan;
   ...
   while (scan < uend && (SIP_URI_ISUNRESERVED (*scan) ||
                          SIP_URI_ISURLESCAPE (scan, uend) || SIP_URI_ISOTHER (*scan) || *scan == '&'))
   {
      ++scan;
   }
   if (scan < uend)
 ...
 * user-unreserved  =  "&" / "=" / "+" / "$" / "," / ";" / "?" / "/"
 */
static void sip_uri_parse_user (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend)
   {
 ...
 * port =  1*DIGIT
 */
static void sip_uri_parse_port (_sip_uri_t * outurl, char *scan, char *uend)
{
   if (scan == uend || *scan != ':' || scan + 1 == uend)
   {
   ...
   if (SIP_URI_ISDIGIT (*scan))
   {
      ...
      {
         outurl->sip_uri_port = outurl->sip_uri_port * 10 + (*scan - '0');
 ...
 */
static int sip_uri_parse_ipv4 (char *scan, char *uend)
{
   int j = 0;
   int val = 0;
 ...
 *  0 if invalid
 */
static int sip_uri_parse_ipv6 (char *scan, char *uend)
{
   char *mark;
   ...
   while (scan < uend && j < 15)
   {
      ...
      if (*scan == '.' && (j == 12 || (zpad != -1 && j < 12)) &&
          mark < uend && sip_uri_parse_ipv4 (mark, uend - 1) && *(uend - 1) == ']')
      {
         mark = uend - 1;
         j += 4;
         scan = mark + 1;
 ...
 * toplabel         =  ALPHA / ALPHA *( alphanum / "-" ) alphanum
 */
static int sip_uri_parse_hostname (char *scan, char *uend)
{
   int sawalpha = 0;
...
static void sip_uri_parse_netpath (_sip_uri_t * outurl, char **pscan, char *uend, boolean_t issip)
{
   char *mark = (char *) 0;
   char *mark2 = (char *) 0;
   char *scan = *pscan;
   ...
   {
      while (scan < uend && ((issip && !SIP_URI_ISSIPHDELIM (*scan)) || (!issip && !SIP_URI_ISABSHDELIM (*scan))))
      {
         ++scan;
      }

      /* look for an IPv4 address */
      if (mark < scan && SIP_URI_ISDIGIT (*mark) && sip_uri_parse_ipv4 (mark, scan))
      {
         gothost = 1;
      }

      /* look for a valid host name */
      if (!gothost && mark < scan && sip_uri_parse_hostname (mark, scan))
      {
         gothost = 1;
      }
   }
   ...
   if (scan < uend && *scan == ':')
   {
      while (scan < uend && ((issip && !SIP_URI_ISSIPDELIM (*scan)) || (!issip && !SIP_URI_ISABSDELIM (*scan))))
      {
         ++scan;
      }
 ...
 */
void sip_uri_parse_it (_sip_uri_t * outurl, sip_str_t * uri_str)
{
   char *mark;
   char *scan;
   char *uend;
   ...
   if ((outurl->sip_uri_scheme.sip_str_len == SIP_SCHEME_LEN &&
       ...
       (outurl->sip_uri_scheme.sip_str_len == SIPS_SCHEME_LEN &&
        !memcmp (outurl->sip_uri_scheme.sip_str_ptr, SIPS_SCHEME, SIPS_SCHEME_LEN)))
   {
   ...
   {                            /* parse absoluteURL */
      ...
      if (scan < uend && *scan == '/')
      {
         ...
         if (outurl->sip_uri_errflags & SIP_URIERR_USER ||
             outurl->sip_uri_errflags & SIP_URIERR_PASS ||
             outurl->sip_uri_errflags & SIP_URIERR_HOST || outurl->sip_uri_errflags & SIP_URIERR_PORT)
         {
            scan = mark;
            while (scan < uend && *scan != '/' && *scan != '?')
            {
               ++scan;
            }
            sip_uri_parse_abs_regname (outurl, mark, scan);
            if (!(outurl->sip_uri_errflags & SIP_URIERR_REGNAME))
            {
               /*
               ...
               outurl->sip_uri_errflags &= ~SIP_URIERR_USER;
               outurl->sip_uri_password.sip_str_ptr = NULL;
               outurl->sip_uri_password.sip_str_len = 0;
               outurl->sip_uri_errflags &= ~SIP_URIERR_PASS;
