...
const struct sip_xaction *sip_get_trans (sip_msg_t sip_msg, int which, int *error)
{
   ...
   return ((sip_transaction_t) sip_xaction_get (NULL, sip_msg, B_FALSE, which, NULL));
}
...
const struct sip_message *sip_get_trans_resp_msg (sip_transaction_t sip_trans, int *error)
{
   ...
   _trans = (sip_xaction_t *) sip_trans;
   if ((_trans->sip_xaction_last_msg != NULL) && !sip_msg_is_request ((sip_msg_t) _trans->sip_xaction_last_msg, error))
   {
      return (_trans->sip_xaction_last_msg);
   }
   else if (!sip_msg_is_request ((sip_msg_t) _trans->sip_xaction_orig_msg, error))
   {
...
char *sip_get_trans_branchid (sip_transaction_t trans, int *error)
{
   ...
   (void) strncpy (bid, xaction->sip_xaction_branch_id, strlen (xaction->sip_xaction_branch_id));
