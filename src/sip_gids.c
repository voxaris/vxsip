...
void sip_md5_hash (char *, int, char *, int, char *, int, char *, int, char *, int, char *, int, uchar_t *);
 ...
 */
static int sip_get_random (char *buf, int buflen)
{
   static int devrandom = -1;

   if (devrandom == -1 && (devrandom = open ("/dev/urandom", O_RDONLY)) == -1)
   {
      return (-1);
   }
...
sip_md5_hash (char *str1, int lstr1, char *str2, int lstr2, char *str3,
              int lstr3, char *str4, int lstr4, char *str5, int lstr5, char *str6, int lstr6, uchar_t * digest)
{
   MD5_CTX ctx;
   ...
   MD5_Update (&ctx, (uchar_t *) & sip_hash_salt, sizeof (uint64_t));

   if (str1 != NULL)
      MD5_Update (&ctx, (uchar_t *) str1, lstr1);

   if (str2 != NULL)
      MD5_Update (&ctx, (uchar_t *) str2, lstr2);

   if (str3 != NULL)
      MD5_Update (&ctx, (uchar_t *) str3, lstr3);

   if (str4 != NULL)
      MD5_Update (&ctx, (uchar_t *) str4, lstr4);

   if (str5 != NULL)
      MD5_Update (&ctx, (uchar_t *) str5, lstr5);

   if (str6 != NULL)
      MD5_Update (&ctx, (uchar_t *) str6, lstr6);
...
char *sip_branchid (sip_msg_t sip_msg)
{
   ...
   if (sip_msg == NULL)
   {
    generate_bid:
      ...
      (void) snprintf (branchid, SIP_BRANCHID_LEN + 1, "z9hG4bK%s", guid);
      free (guid);
      return (branchid);
   }
   ...
   if (_sip_msg->sip_msg_req_res == NULL ||
       _sip_msg->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_ptr == NULL)
   {
      return (NULL);
   }
   ...
   MD5_Update (&ctx, (uchar_t *) via->sip_hdr_start, hdrlen);
   hdrlen = to->sip_hdr_end - to->sip_hdr_start;
   MD5_Update (&ctx, (uchar_t *) to->sip_hdr_start, hdrlen);
   hdrlen = from->sip_hdr_end - from->sip_hdr_start;
   MD5_Update (&ctx, (uchar_t *) from->sip_hdr_start, hdrlen);
   hdrlen = callid->sip_hdr_end - callid->sip_hdr_start;
   MD5_Update (&ctx, (uchar_t *) callid->sip_hdr_start, hdrlen);
   MD5_Update (&ctx, (uchar_t *) _sip_msg->sip_msg_req_res->
                    U.sip_request.sip_request_uri.sip_str_ptr,
                    _sip_msg->sip_msg_req_res->U.sip_request.sip_request_uri.sip_str_len);
   MD5_Update (&ctx, (uchar_t *) & cseq, sizeof (int));
   ...
   {
      (void) snprintf (&branchid[2 * i], len - (2 * i), "%02x", md5_hash[i]);
   }
   return (branchid);
}
