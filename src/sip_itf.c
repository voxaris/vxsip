...
void (*sip_ulp_recv) (const sip_conn_object_t, sip_msg_t, const sip_dialog_t) = NULL;
uint_t (*sip_stack_timeout) (void *, void (*func) (void *), struct timeval *) = NULL;
boolean_t (*sip_stack_untimeout) (uint_t) = NULL;
void (*sip_ulp_dlg_del) (sip_dialog_t, sip_msg_t, void *) = NULL;
int (*sip_stack_send) (sip_conn_object_t xonn_object, char *, int) = NULL;
...
int (*sip_conn_rem_addr) (sip_conn_object_t, struct sockaddr *, socklen_t *) = NULL;
int (*sip_conn_local_addr) (sip_conn_object_t, struct sockaddr *, socklen_t *) = NULL;
...
void sip_send_resp_err (sip_conn_object_t conn_obj, _sip_msg_t * sip_msg, int resp)
{
   _sip_msg_t *sip_msg_resp;

   sip_msg_resp = (_sip_msg_t *) sip_create_response ((sip_msg_t) sip_msg, resp, sip_get_resp_desc (resp), NULL, NULL);
   ...
   (void) sip_stack_send (conn_obj, sip_msg_resp->sip_msg_buf, sip_msg_resp->sip_msg_len);
}

/* Validate some of the common headers */
boolean_t sip_check_common_headers (sip_conn_object_t conn_obj, _sip_msg_t * sip_msg)
{
   int err;
   ...
   return (B_FALSE);
 error:
   sip_send_resp_err (conn_obj, sip_msg, SIP_BAD_REQUEST);
   return (B_TRUE);
}
 ...
 */
int sip_setup_header_pointers (_sip_msg_t * sip_msg)
{
   char *msg;
   ...
   for (;;)
   {
      ...
      if (strncmp (SIP_CRLF, msg, strlen (SIP_CRLF)) == 0)
      {
         ...
         sip_msg_header->sip_hdr_prev = sip_msg->sip_msg_headers_end;
         sip_msg_header->sip_hdr_next = NULL;
         sip_msg_header->sip_hdr_sipmsg = sip_msg;
         sip_msg->sip_msg_headers_end->sip_hdr_next = sip_msg_header;
   ...
   sip_msg->sip_msg_headers_start = sip_msg->sip_msg_headers_start->sip_hdr_next;
   ...
   sip_msg->sip_msg_content->sip_content_end = sip_msg->sip_msg_buf + sip_msg->sip_msg_len;
   sip_msg->sip_msg_content->sip_content_allocated = B_FALSE;
   sip_msg->sip_msg_content_len =
      sip_msg->sip_msg_content->sip_content_end - sip_msg->sip_msg_content->sip_content_start;
   return (0);
}
 ...
 */
int sip_sendmsg (sip_conn_object_t obj, sip_msg_t sip_msg, sip_dialog_t dialog, uint32_t flags)
{
    ...
    * necessary here?)
    */
   if (stateful && (!sip_msg_info->is_request || sip_msg_info->sip_req_method != ACK))
   {
   ...
   if (sip_manage_dialog)
   {
      ...
      {
         dialog = (sip_dialog_t) sip_seed_dialog (obj, sip_msg, sip_ulp_dlg_del, dlg_on_fork, SIP_UAC_DIALOG);
      }
      else if (dialog != NULL && (!sip_msg_info->is_request || sip_msg_info->sip_req_method == NOTIFY))
      {
         (void) sip_update_dialog (dialog, _sip_msg, sip_ulp_dlg_del);
      }
   }

   if ((ret = sip_stack_send (obj, _sip_msg->sip_msg_buf, _sip_msg->sip_msg_len)) != 0)
   {
      if (sip_trans != NULL)
      {
         sip_xaction_terminate (sip_trans, _sip_msg, sip_conn_transport (obj));
      }
      sip_refrele_conn (obj);
      return (ret);
   }
 ...
 */
boolean_t sip_valid_sent_by (sip_msg_t sip_msg)
{
   sip_header_t via;
...
void sip_process_new_packet (sip_conn_object_t conn_object, void *msgstr, size_t msglen)
{
   ...
   {
    next_msg:
      msgstr = (char *) sip_get_tcp_msg (conn_object, (char *) msgstr, &msglen);
      if (msgstr == NULL)
      {
   ...
   if (sip_manage_dialog)
   {
      ...
      if (dialog == NULL)
      {
         if (sip_msg_info->is_request)
         {
             ...
             */
            dialog = (sip_dialog_t) sip_seed_dialog (conn_object, sip_msg, sip_ulp_dlg_del, B_FALSE, SIP_UAS_DIALOG);
         }
         else
         {
            dialog = sip_dialog_create (sip_msg, NULL, SIP_UAC_DIALOG);
         }
         dialog_created = B_TRUE;
      }
      else if (sip_incomplete_dialog (dialog) && (!sip_msg_info->is_request || sip_msg_info->sip_req_method == NOTIFY))
      {
         dialog = sip_update_dialog (dialog, sip_msg, sip_ulp_dlg_del);
      }
      else if (sip_dialog_process (sip_msg, &dialog, sip_ulp_dlg_del) != 0)
      {
         if (dialog != NULL)
...
int sip_stack_init (sip_stack_init_t * stack_val)
{
   ...
   if (sip_stack_send != NULL || stack_val->sip_version != SIP_STACK_VERSION)
   {
      return (EINVAL);
   }
   if (stack_val->sip_io_pointers == NULL || stack_val->sip_ulp_pointers == NULL)
   {
      return (EINVAL);
   }
   ...
   sip_conn_local_addr = stack_val->sip_io_pointers->sip_conn_local_address;
   ...
   if (sip_ulp_recv == NULL || sip_stack_send == NULL ||
       ...
       sip_conn_rem_addr == NULL || sip_conn_local_addr == NULL || sip_conn_transport == NULL)
   {
    err_ret:
      sip_ulp_recv = NULL;
      sip_stack_send = NULL;
   ...
   if (stack_val->sip_ulp_pointers->sip_ulp_timeout != NULL)
   {
      if (stack_val->sip_ulp_pointers->sip_ulp_untimeout == NULL)
         goto err_ret;
      sip_stack_timeout = stack_val->sip_ulp_pointers->sip_ulp_timeout;
      sip_stack_untimeout = stack_val->sip_ulp_pointers->sip_ulp_untimeout;
   }
   else
   {
      if (stack_val->sip_ulp_pointers->sip_ulp_untimeout != NULL)
   ...
   if (sip_manage_dialog)
   {
      if (stack_val->sip_ulp_pointers->sip_ulp_dlg_del != NULL)
      {
         sip_ulp_dlg_del = stack_val->sip_ulp_pointers->sip_ulp_dlg_del;
      }
      sip_dialog_init (stack_val->sip_ulp_pointers->sip_ulp_dlg_state_cb);
   }
